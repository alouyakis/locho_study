...
date: 2025.01.31
author: 
 - Matt Jackson (study design and execution)
 - Dayakar Badri (data generation)
 - Nicole Green (data generation)
 - Tobyn Bernazzani (analysis - abr method)
 - Artemis S. Louyakis (analysis)
...
tables_dir <- "tables"
noiseq_dir <- "noiseq"
noiseq_plots_dir <- "plots/noiseq"
network_plots_dir <- "plots/network"
kegg_maps_dir <- "plots/kegg_maps"
...
if (!dir.exists(noiseq_plots_dir)) {
  dir.create(noiseq_plots_dir)
}
if (!dir.exists(network_plots_dir)) {
  dir.create(network_plots_dir)
}
if (!dir.exists(kegg_maps_dir)) {
  dir.create(kegg_maps_dir)
}
```
...
write.table(count_totals, paste0(tables_dir, "/seq_counts/count_totals.csv"), quote = FALSE, col.names = FALSE)
```
...
# len <- read.delim(paste0(data_dir, "/gene_length.txt"))
len <- read.delim(paste0(data_dir, "/isoform_length.txt"))
...
metacyc_map <- read.delim("annotations/uniprot_to_metacyc.txt", header = FALSE, col.names = c("Entry", "mapped", "metacyc")) %>%
  mutate(Entry = str_replace(Entry, 'UniProt:', '')) %>%
  filter(mapped == 1) %>%
  select(Entry, metacyc)
...
mg_metadata <- read.delim(paste0(data_dir, "/mg_metadata.csv")) %>%
  rename_with(~ tolower(gsub(".", "_", .x, fixed = TRUE))) %>%
  mutate(mg = sub("^([^-]*-[^-]*).*", "\\1", metagenomics_sample_number))

mt_metadata <- read.delim(paste0(data_dir, "/mt_sample_data.txt")) %>%
  rename_with(~ tolower(gsub(".", "_", .x, fixed = TRUE))) %>%
  mutate(mt = sub("^([^-]*-[^-]*).*", "\\1", sample_number)) %>%
  filter(mt != "F-50179103") ## MONSTER only sampled for single time point

mgmt_map <- read.delim(paste0(data_dir, "/bb4_mtmg_map.tsv")) %>%
  dplyr::rename(mt_full = X.mt, mg_full = mg) %>%
  mutate(mg = sub("^([^-]*-[^-]*).*", "\\1", mg_full),
         mt = sub("^([^-]*-[^-]*).*", "\\1", mt_full)) %>%
  filter(mt != "F-50179103")

# sample_data <- read.delim(paste0(data_dir, "/locho_105536_sample_data.tsv"))
# results_data <- read.delim(paste0(data_dir, "/locho_105536_results_data.tsv"))
...
xs <- summary_trinotate(x)
xs

sink(paste0(tables_dir, "/annotation_summary.txt"))
cat("Total # of genes:", ag, "\n")
cat("Total # of isoforms:", ai, "\n")
cat("Total # of proteins:", ap, "\n")
cat("Total # with blastx or blastp hit:", b, "\n")
...
write.table(xs, file = paste0(tables_dir, "/trinotate_summary.csv"), 
            quote = FALSE, sep = "\t", row.names = TRUE, col.names = NA)

rm(ag, ai, ap, b, c, d, e, f, x, xs)
```
...
write.table(counts_up, paste0(tables_dir, "/rsem_uniprot.csv"), quote = FALSE, row.names = TRUE)
...
tpm_up <- merge(trinotate_up, tpm, by.x = "transcript_id", by.y = "row.names") %>%
  ...
  dplyr::summarise(across(where(is.numeric), ~ sum(.x, na.rm = TRUE))) %>%
  column_to_rownames(var = "entry")
write.table(tpm_up, paste0(tables_dir, "/tpm_uniprot.csv"), quote = FALSE, row.names = TRUE)
...
write.table(counts_ko, paste0(tables_dir, "/rsem_ko.csv"), quote = FALSE, row.names = TRUE)
...
tpm_ko <- tpm_up %>%
  ...
  dplyr::summarise(across(everything(), sum)) %>%
  column_to_rownames("ko")
write.table(tpm_ko, paste0(tables_dir, "/tpm_ko.csv"), quote = FALSE, row.names = TRUE)
...
write.csv(track_counts, paste0(tables_dir, "/seq_counts/functional_counts.csv"), quote = FALSE, row.names = FALSE)
```
...
pca + theme_bw()
ggsave(paste0(plots_dir, "/clustering/pca_kegg_rsem.png"))
...
# Plot heatmap
pheatmap(vst_cor, cellheight = 10, cellwidth = 10, filename = paste0(plots_dir, "/clustering/heatmap_kegg_rsem.png"))
...
ggplot2::ggsave(paste0(plots_dir, "/clustering/clusters_kegg_p0.05_rsem.png"), height = 10, width = 14, create.dir = TRUE)
# ggplot2::ggsave(paste0(plots_dir, "/clustering/clusters_kegg_p1.0_rsem.png"), height = 8, width = 14, create.dir = TRUE)
...
lapply(names(dfs),
       function(x){write.csv(dfs[[x]], paste0(tables_dir, "/clusters/cluster_kegg_p0.05_rsem_", x,".csv"),
       # function(x){write.csv(dfs[[x]], paste0(tables_dir, "/clusters/cluster_kegg_p1.0_rsem_", x,".csv"),
                             row.names = FALSE)})
```

differential expression analysis for kegg
...
save(dds, file = paste0(noiseq_dir, "/dds_ko.RData"))
  ...
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        ...
        save(tmm, file = paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm.RData"))
        tmm_r <- as.data.frame(tmm@results)
        write.table(tmm@results, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        tmm_r <- na.omit(tmm_r)
        tmm_r$P <- with(tmm_r, 1-prob)
        tmm_r$id <- row.names(tmm_r)
        write.table(tmm_r, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        tmm.deg <- list(all = degenes(tmm, q = 0.95, M = NULL),
                        up = degenes(tmm, q = 0.95, M = "up"),
                        down = degenes(tmm, q = 0.95, M = "down"))
        write.table(tmm.deg$all, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_deg.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        write.table(tmm.deg$down, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_deg_down.txt"), sep = "\t",
            row.names = TRUE, col.names = NA, quote = FALSE)
        write.table(tmm.deg$up, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_deg_up.txt"), sep = "\t",
            row.names = TRUE, col.names = NA, quote = FALSE)
      }}}}
```
...
for(s1 in treatment){
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        print(c(s1,s2))
        load(paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm.RData"))
        DE.plot(tmm, graphic = "MD", q = 0.95)
        tmm_r <- as.data.frame(tmm@results)
        ## plot with ggplot - 
        tmp <- data.frame(log2FC = tmm_r[,5], log_c = log(tmm_r[,1]), log_s = log(tmm_r[,2]), prob = tmm_r[,4])
        tmp$abs_mean_diff <- rowMeans(tmp[,c("log_c", "log_s")])
        tmp <- tmp %>% mutate(p = case_when(
          prob >= 0.95 ~ "sig",
          prob < 0.95 ~ "not sig"
          ))
        ggplot(tmp, aes(x = log2FC, y = abs_mean_diff)) +
          geom_point(aes(color = p), alpha = 0.5) +
          ...
          theme(legend.position = "none")
        ggsave(paste0(noiseq_plots_dir, "/md_",s1,"_vs_",s2,"_ko.png"))
        tmm_r
      }}}}
```
...
for(s1 in treatment){
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        print(c(s1,s2))
        tmm_r <- read.delim(paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered.txt"), 
        ...
        write.table(tmm_rm, paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered_annot.txt"), 
        ...
        volc_plot <- volcanoly(volc_obj, effect_size_line = c(-2,2), effect_size_line_color = "orange",
                               genomewideline = -log10(1e-2), genomewideline_color = "green", title = paste(s1, "vs", s2, "(kegg)"))
        # print(volc_plot)
        htmlwidgets::saveWidget(volc_plot, paste0(noiseq_plots_dir, "/volcano_ko_nsb_",s1,"_vs_",s2,"_tmm_filtered.html"))
      }}}}
```

differential expression for uniprot
```{r differential expression - uniprot}
Sys.time()
...
## cpm normalization
up_cpm <- cpm(counts_up)

## create noiseq object
dds <- NOISeq::readData(data = up_cpm, factors = metadata)
save(dds, file = paste0(noiseq_dir, "/dds_uniprot.RData"))
  ...
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        ...
        tmm <- noiseqbio(dds, norm = "tmm", k = 0.5, factor = "treatment", conditions = c(s1, s2), 
                             r = 50, filter = 0, a0per= 0.9, nclust = 15, random.seed = 321, plot = TRUE, lc = 0)
        save(tmm, file = paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm.RData"))
        tmm_r <- as.data.frame(tmm@results)
        write.table(tmm@results, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        tmm_r <- na.omit(tmm_r)
        tmm_r$P <- with(tmm_r, 1-prob)
        tmm_r$id <- row.names(tmm_r)
        write.table(tmm_r, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_filtered.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        tmm.deg <- list(all = degenes(tmm, q = 0.95, M = NULL),
                        up = degenes(tmm, q = 0.95, M = "up"),
                        down = degenes(tmm, q = 0.95, M = "down"))
        write.table(tmm.deg$all, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_deg.txt"), sep = "\t",
                    row.names = TRUE, col.names = NA, quote = FALSE)
        write.table(tmm.deg$down, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_deg_down.txt"), sep = "\t",
            row.names = TRUE, col.names = NA, quote = FALSE)
        write.table(tmm.deg$up, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_deg_up.txt"), sep = "\t",
            row.names = TRUE, col.names = NA, quote = FALSE)
      }}}}
```

```{r md plots for uniprot}
Sys.time()
...
for(s1 in treatment){
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        print(c(s1,s2))
        load(paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm.RData"))
        DE.plot(tmm, graphic = "MD", q = 0.95)
        tmm_r <- as.data.frame(tmm@results)
        ## plot with ggplot - 
        tmp <- data.frame(log2FC = tmm_r[,5], log_c = log(tmm_r[,1]), log_s = log(tmm_r[,2]), prob = tmm_r[,4])
        ...
        ggplot(tmp, aes(x = log2FC, y = abs_mean_diff)) +
          ...
          labs(x = expression("Log"["2"]*"FC"),
            y = "Absolute value of difference of means",
            title = paste("MD plot (uniprot):",s1,"vs",s2)) +
          scale_color_manual(values=c("black", "red")) +
          theme(legend.position = "none")
        ggsave(paste0(noiseq_plots_dir, "/md_",s1,"_vs_",s2,"_uniprot.png"))
        tmm_r
      }}}}
```

```{r interactive volcano - uniprot}
Sys.time()
...
for(s1 in treatment){
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        print(c(s1,s2))
        tmm_r <- read.delim(paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_filtered.txt"), 
                            header=TRUE, row.names = 1, com = '', check.names = FALSE)
        tmm_rm <- merge(tmm_r, uniprot_map, by.x = "id", by.y = "Entry", all.x = TRUE)
        tmm_rm$EFFECTSIZE <- tmm_rm$log2FC
        write.table(tmm_rm, paste0(noiseq_dir, "/nsb_uniprot_",s1,"_vs_",s2,"_tmm_filtered_annot.txt"), 
                    sep = "\t", row.names = FALSE, quote = FALSE)
        volc_obj <- volcanor(tmm_rm, p = "P", effect_size = "EFFECTSIZE", snp = "id", 
                             gene = "EC.number", annotation1 = "Gene.Names", annotation2 = "Protein.names")
        volc_plot <- volcanoly(volc_obj, effect_size_line = c(-2,2), effect_size_line_color = "orange",
                               genomewideline = -log10(1e-2), genomewideline_color = "green", title = paste(s1, "vs", s2, "(uniprot)"))
        # print(volc_plot)
        htmlwidgets::saveWidget(volc_plot, paste0(noiseq_plots_dir, "/volcano_uniprot_nsb_",s1,"_vs_",s2,"_tmm_filtered.html"))
      }}}}
```
...
plot <- plot_network(ig, physeq, type = 'taxa', color = 'level2', label = NULL,
        ...
        theme(legend.position = "none")
ggsave(plot, paste0(network_plots_dir, "/all_level2_dist0.8.png"))

# label all points
plot <- plot + geom_text(aes_string(label="value"), size = 2, hjust=1.35, na.rm=TRUE)
ggsave(plot, paste0(network_plots_dir, "/all_level2_dist0.8_labeled.png"))

## make with legends
plot <- plot_network(ig, physeq, type = 'taxa', color = 'level2', label = NULL,
             line_alpha = 0.1, point_size = 1, line_weight = 0.1)
ggsave(plot, paste0(network_plots_dir, "/all_level2_dist0.8_wlegend.png"))
```
...
## graph subset
ig_sub <- make_network(physeq_sub, type = "taxa", distance = "bray", max.dist = 0.8, keep.isolates = FALSE)

## plot the network and save the basic plot without a legend
plot <- plot_network(ig_sub, physeq_sub, type = 'taxa', color = 'level3', label = NULL,
                     line_alpha = 0.1, point_size = 1, line_weight = 0.1) +
        theme(legend.position = "none")
plot
ggsave(paste0(network_plots_dir, "/energy_level3_dist0.8.png"))

# label all points
plot + geom_text(aes_string(label="value"), size = 2, hjust=1.35, na.rm=TRUE)
ggsave(paste0(network_plots_dir, "/energy_level3_dist0.8_labeled.png"))

## make with legends
plot_network(ig_sub, physeq_sub, type = 'taxa', color = 'level3', label = NULL,
             line_alpha = 0.1, point_size = 1, line_weight = 0.1)
ggsave(paste0(network_plots_dir, "/energy_level3_dist0.8_wlegend.png"))
```
...
antibiotic resistance filter
```{r abr}
Sys.time()

## read in the brite to ko table and filter for antibiotic resistance
ab_kos <- read.delim("/data/databases/kegg/ftp.pathway.jp/kegg/brite/links/brite_ko.list", 
                     header = FALSE, col.names = c("brite", "ko")) %>%
  filter(brite == "br:ko01504") %>%
  select(ko) %>%
...
## filter de tables for abr genes and plot 
for(s1 in treatment){
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        ...
        tmm_r <- read.delim(paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered.txt"), 
                            header=TRUE, row.names = 1, com = '', check.names = FALSE)
        # tmm_r <- filter(tmm_r, id %in% ab_kos$ko) ## redundant
        tmm_rm <- merge(tmm_r, khier, by.x = "id", by.y = "ko", all.x = TRUE) %>%
          filter(level3 == "01504 Antimicrobial resistance genes [BR:ko01504]") %>%
          mutate(color = ifelse(log2FC > 0 & prob >= 0.95, "red",
                                ifelse(log2FC > 0 & prob < 0.95, "pink",
                                       ifelse(log2FC < 0 & prob >= 0.95, "blue",
                                              "lightblue")))) %>%
          mutate(sig = ifelse(prob >= 0.95, "sig", "notsig")) %>%
          mutate(label_hjust = ifelse(log2FC >= 0, 1, 0),
                 label_nudge = ifelse(log2FC >= 0, -0.05, 0.05)) %>%
          arrange(log2FC)
        write.table(tmm_rm, paste0(noiseq_dir, "/nsb_ko_", s1, "_vs_", s2, "_tmm_filtered_annot_abr.txt"),
                    sep = "\t", row.names = FALSE, quote = FALSE)
        p <- ggplot(tmm_rm, aes(x = log2FC, y = reorder(id, -log2FC), fill = sig)) +
        geom_bar(stat = 'identity') +
          theme_classic() +
          labs(y = "KEGG Ortholog", x = expression("log"["2"]*" fold change"), 
               title = paste0("← higher in ", s1, " | Antimicrobial resistance genes | higher in ", s2, " →")) +
          theme(plot.title = element_text(hjust = 0.5)) +
          scale_fill_manual(values = c("notsig" = "black", 
                                      "sig" = "red")) +
          theme(legend.position = "none") +
          xlim(-round(max(abs(tmm_r$log2FC))), round(max(abs(tmm_r$log2FC)))) +
          geom_text(aes(x = 0, label = ko_name, hjust = label_hjust), size = 2, nudge_x = tmm_rm$label_nudge)
        print(p)
        ggsave(paste0(noiseq_plots_dir, "/nsb_pathway_abrgenes_" , s1, "_vs_", s2, ".png"), 
               dpi = 300, width = 10, height = 14, units = "in")
      }}}}
...
count number of genes present in pathway vs total genes in pathway
```{r create pathway ko counts table}
Sys.time()

## import, filter, and reformat pathway-ko list
ko_pwy_list <- read.delim("/data/databases/kegg/ftp.pathway.jp/kegg/pathway/ko/ko.list",
                          header = FALSE, col.names = c("pwy_id", "ko")) 
ko_pwy_list <- ko_pwy_list %>% 
  unique() %>%
  filter(str_detect(ko, "^ko:K")) %>%
  mutate(ko = gsub("ko:", "", ko)) %>%
  mutate(pwy_id = gsub("path:ko", "", pwy_id))

## count totals
ko_pwy_counts <- ko_pwy_list %>% 
  dplyr::count(pwy_id, name = "total") 
  ...
  for(s2 in treatment){
    if(s1!=s2){
      if(s1<s2){
        ...
        tmm_r <- read.delim(paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered.txt"), 
                            header=TRUE, row.names = 1, com = '', check.names = FALSE)
        expressed_kos <- unique(tmm_r$id)
        expressed_in_pathways <- ko_pwy_list[ko_pwy_list$ko %in% expressed_kos, ]
        counts <- as.data.frame(table(expressed_in_pathways$pwy_id))
        comp_name <- paste0(s1, "_vs_", s2)
        colnames(counts) <- c("pwy_id", comp_name)
        ko_pwy_counts <- merge(ko_pwy_counts, counts, by="pwy_id", all.x=TRUE)
        ko_pwy_counts[[comp_name]][is.na(ko_pwy_counts[[comp_name]])] <- 0
      }}}}

## import and reformat pathway names
pwy_names <- read.delim("/data/databases/kegg/ftp.pathway.jp/kegg/pathway/map_title.tab",
                        header = FALSE, col.names = c("pwy_id", "pwy_name"), colClasses = "factor")
pwy_names <- pwy_names %>%
  unique() 

## merge counts table with pathway names
ko_pwy_counts <- merge(pwy_names, ko_pwy_counts, by = "pwy_id", all.y = TRUE)

## write table
write.table(ko_pwy_counts, paste0(tables_dir, "/pwy_ko_tx_counts.csv"), quote = FALSE, row.names = FALSE, 
            sep = "\t")

```

DE Barplots - all pathways
```{r}
Sys.time()
...
DE plotted onto KEGG pathway maps
TODO: convert to use /data/databases/kegg/ftp.pathway.jp/kegg/xml/ files from ftp instead of downloading from kegg
TODO: try /data/databases/kegg/ftp.pathway.jp/kegg/xml/kgml+/ for global maps that didn't work
```{r kegg maps}
Sys.time()

## read in the brite to ko table and filter for antibiotic resistance
pathway_list <- read.delim("/data/databases/kegg/ftp.pathway.jp/kegg/pathway/pathway.list", 
                     header = FALSE, col.names = c("pwy_id", "pwy_name"), comment.char = "#", colClasses = "factor") 

## make list of pathway ids
pwy_ids <- as.vector(pathway_list$pwy_id)
# pwy_ids <- c("00010", "00020")
    ...
    if(s1!=s2){
      if(s1<s2){
        ...
        tmm_r <- read.delim(paste0(noiseq_dir, "/nsb_ko_",s1,"_vs_",s2,"_tmm_filtered.txt"), 
          ...
          as.matrix()
        setwd(kegg_maps_dir)
        for(i in 1:length(pwy_ids)){
          filename <- paste0("ko", pwy_ids[i], ".", s1, "_vs_", s2, ".png")
          if(!file.exists(filename)){
            tryCatch({
              pv_out <- pathview(gene.data = tmm_r,
                   pathway.id = pwy_ids[i],
                   species = "ko",
                   out.suffix = paste0(s1, "_vs_", s2),
                   low = "#46ACC8",
                   mid = "grey",
                   high = "#E58601")
          ...
          files <- list.files(pattern = ".nfs*", all.files = TRUE)
...
## 37 maps weren't created for any of the samples;
## 2 additional maps weren't created for T1_LoCHO_PROT_vs_T2_LoCHO_FAT
```
...
metaphlan <- readxl::read_excel(paste0(data_dir, "/msystems.00452-24-s0004.xlsx"))
...
write.csv(species_table, paste0(tables_dir, "/metaphlan_species.csv"), quote = FALSE)
```
