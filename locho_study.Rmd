...
```{r aggregate counts by function}
Sys.time()
...
track_counts <- colSums(counts_up)
```
...
count_data <- round(humann_ko, 0)
# count_data <- count_data[rowSums(count_data) > 10,]
dds <- DESeqDataSetFromMatrix(countData = count_data, colData = metadata, design = ~ phase_day)
# Wald test
...
# Plot PCA 
plotPCA(vst, intgroup = "phase_day")
...
cluster_all <- degPatterns(cluster_vst_all, metadata = metadata2, time = "day", col = "phase", minc = 1)

#### ggplot manually
ggplot(cluster_all$plot$data) +
  geom_line(aes(x = phase_day, y = value, group = genes), color = "grey") +
  geom_point(position = "jitter", aes(x = phase_day, y = value, 
                                    alpha = 0.2, color = phase
                                    )) +
  geom_boxplot(aes(x = phase_day, y = value, color = phase,
                   alpha = 0.2
                   )) +
  theme_bw() +
...
ggsave("clustering/clusters.png", height = 14, width = 14, create.dir = TRUE)
...
lapply(names(dfs),
       function(x){write.csv(dfs[[x]], paste0("clustering/kegg_", x,".csv"),
                             row.names = FALSE)})
```
